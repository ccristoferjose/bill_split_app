...
const TYPE_META = {
  initial_balance:        { color: 'text-indigo-600',   bg: 'bg-indigo-50',   label: 'Starting balance' },
  ...
  manual_adjustment:      { color: 'text-gray-600 dark:text-muted-foreground',   bg: 'bg-gray-50 dark:bg-muted/30',   label: 'Adjustment' },
};
  ...
  return (
    <Card className="overflow-hidden">
      <CardContent className="p-0">
        <div className="p-4 sm:p-5 bg-gradient-to-br from-indigo-50 via-white to-emerald-50 dark:from-indigo-950/40 dark:via-card dark:to-emerald-950/40">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-gray-500 dark:text-muted-foreground">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="h-7 px-2 text-xs text-gray-500 dark:text-muted-foreground"
              onClick={() => setOpen(o => !o)}
            >
          ...
          <div className={`mt-1 text-3xl sm:text-4xl font-semibold ${isNegative ? 'text-rose-600' : 'text-gray-900 dark:text-foreground'}`}>
        ...
        {open && (
          <div className="border-t bg-white dark:bg-card">
            {(!historyData?.entries || historyData.entries.length === 0) ? (
              <p className="p-4 text-sm text-gray-500 dark:text-muted-foreground text-center">
                {t('balance.empty', 'No ledger entries yet.')}
              </p>
            ) : (
              <ul className="divide-y max-h-72 overflow-y-auto">
                {historyData.entries.map(e => {
                  const meta = TYPE_META[e.entry_type] || { color: 'text-gray-700 dark:text-foreground/90', bg: 'bg-gray-50 dark:bg-muted/30', label: e.entry_type };
                    ...
                    <li key={e.id} className="flex items-center justify-between px-4 py-2.5">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm text-gray-800 dark:text-foreground truncate">
                            {e.description || meta.label}
                          </div>
                          <div className="text-[11px] text-gray-400 dark:text-muted-foreground/70">
