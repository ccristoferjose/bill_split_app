...
import { Receipt, CalendarDays, ArrowDownCircle } from 'lucide-react';
...
const EXPENSE_CATEGORIES = [
  ...
  'Health', 'Travel', 'Subscriptions', 'Utilities', 'Other',
];

const INCOME_CATEGORIES = ['Salary', 'Freelance', 'Refund', 'Bonus', 'Other'];
...
const CreateTransactionModal = ({ isOpen, onClose, userId }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    recurrence:   '',
    notes:        '',
    participants: [], // [{ userId, username, amount }]
  });
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Category — Expense (required) and Income (optional) */}
          {showCategory && (
            <div
              className="transition-all duration-200 animate-in fade-in slide-in-from-top-1"
            >
          ...
          {/* Split with Friends — Bill (always shown) and Expense (optional) */}
          {showSplit && (
            <div
              className="transition-all duration-200 animate-in fade-in slide-in-from-top-1"
            >
